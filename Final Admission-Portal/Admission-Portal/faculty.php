...
<title>Admission Portal (Staff)</title>
<script language=JavaScript>
function reload()
{
var s=form.searchbox.options[form.searchbox.options.selectedIndex].value;
self.location='faculty.php?s=' + s;
}
</script>
<style>
table.border
...
left:0px;top:100px;width:174px;height:105px'><img width=174 height=1800
...
<span style='position:absolute;z-index:15;left:-220px;top:50px;width:136px;height:136px'></span>
<div style='position:absolute;margin-left:700px;margin-top:-100px;width:384px;
height:26px;z-index:224'><font face="Ubuntu">
Welcome <strong><?php echo $_SESSION['emp_no'];?></strong>  |  <a href="flogout.php">Logout</a>
</div>  
<select id=="searchbox" name="searchbox" size="1">
  <option value="">---Select Registration Number---</option>
  <?php
  include('dbconnect.php');
  $search=mysqli_query($con, "select reg_no from confirm");
  while($row=mysqli_fetch_array($search))
  {
    if($row['reg_no']==$_GET['s'])
	{
	echo "<option selected value='$row[reg_no]'>$row[reg_no]</option>";
	}
	else
	{
...
<input type="button" name="search" class="button" onclick="reload()" value="Search"></br></br>
<?php 
if($_GET['s']!="")
	include('f_print.php') 
?>
